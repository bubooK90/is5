      ...
      <Masonry breakpointCols={breakpointColumnsObj} className="my-masonry-grid" columnClassName="my-masonry-grid_column">
        ...
        {items.map((item, index) => {
          if (index === items.length - 1) {
            return (
              <div className='bg-stone-900/10 rounded-lg' ref={lastItemRef} key={index} onClick={() => handleCardClick(item)}>
                <img
            ...
            return (
              <div className='bg-stone-900/10 rounded-lg' key={index} onClick={() => handleCardClick(item)}>
                <img
                  className="lazy rounded-lg cursor-pointer"
                  src={`https://cdn.midjourney.com/${item.jobid}/${item.index}.png`}
                  alt={`${item.prompt}`}
